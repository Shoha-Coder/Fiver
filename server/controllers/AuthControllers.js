  ...
  try {
    if (req?.userId) {
      ...
      if (userName && fullName && description) {
        ...
        return res.status(200).send("Profile data updated succesfully");
      }
    }else{
      return res
      .status(400)
      .send("Username, Fullname and description should be included")
    }
  } catch (err) {
...
export const setUserImage = async (req, res, next) => {
  try{
    if(req.file) {
      if(req.userId){
        const date = Date.now()
        let fileName = "uploads/profiles/" + date + extname(req.file.originalname)
    ...
    return res.status(400).send("Image not icluded")
  }catch(err){
